...
from forms.test_solve import TestSolveForm, QuestionSolveForm
...
def teacher_delete(id):
    ...
    if teacher and teacher in current_user.teachers:
        curr2 = db_sess.merge(current_user)
...
def student_delete(id):
    ...
    if student and student in current_user.students:
        curr2 = db_sess.merge(current_user)
...
def test_attempt(id):
    ...
    if request.method == "GET":
        ...
        attempt.test = test
        attempt.start_date = datetime.now().strftime('%d/%m/%y %H:%M:%S.%f')
    ...
    if request.method == "POST":
        ...
        if len(answers) > 0:
            ...
            attempt.answers = ''
        attempt.complete_time = datetime.now().strftime('%d/%m/%y %H:%M:%S.%f')
        db_sess.commit()
...
def attempt_result(id):
    ...
    delta = str(datetime.strptime(attempt.complete_time, '%d/%m/%y %H:%M:%S.%f') -
                datetime.strptime(attempt.start_date, '%d/%m/%y %H:%M:%S.%f'))
...
def teachers():
    ...
    if current_user.__class__.__name__ == 'Student':
        form = TeacherAddForm()
        teachers = db_sess.query(Teacher).filter(Teacher.id.in_(teacher.id for teacher in current_user.teachers))
        if request.method == "POST":
            if form.validate_on_submit():
                ...
                if new_teacher in current_user.teachers:
                    return render_template("teachers.html", teachers=teachers,
                                           message='Вы уже добавили этого учителя', form=form)
                current_user2 = db_sess.merge(current_user)
...
def students():
    db_sess = db_session.create_session()
    if current_user.__class__.__name__ == 'Teacher':
        students = db_sess.query(Student).filter(Student.id.in_(student.id for student in current_user.students))
    else:
        abort(404)
...
def show_attempts():
    ...
    if current_user.is_authenticated:
        ...
        if current_user.__class__.__name__ == 'Student':
            attempts = db_sess.query(Attempt).join(Test).join(Teacher).filter(Teacher.id.in_(teacher.id for teacher in current_user.teachers), Test.is_private != True)
...
def index():
    ...
    if current_user.is_authenticated:
        ...
        if current_user.__class__.__name__ == 'Student':
            ...
            tests = db_sess.query(Test).join(Teacher).filter(Teacher.id.in_(teacher.id for teacher in current_user.teachers), Test.is_private != True)
